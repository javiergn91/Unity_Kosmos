...
namespace Kosmos.Utility
{
    static public class Easings
    {
        ...
        static public Vector3 Interpolate(Vector3 s, Vector3 f, float t, Functions func = Functions.Linear)
        {
            float tF = Interpolate(t, func);

            return new Vector3(s.x * (1 - tF) + f.x * tF, s.y * (1 - tF) + f.y * tF, s.z * (1 - tF) + f.z * tF);
        }
